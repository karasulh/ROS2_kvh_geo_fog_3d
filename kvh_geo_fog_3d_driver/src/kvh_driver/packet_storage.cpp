...
/**
   ...
   * @return [int]:
   * 
   *    1 = Warning, duplicate id found
   *    0 = Success,
   *    -1 = Failure, unsupported id found
   */
int KvhPacketStorage::Init(KvhPacketRequest &_packRequest)
{
  ...
  for (int i = 0; (std::size_t)i < _packRequest.size(); i++)
  {
    ...
    // Returns immediately on error, or with warning if duplicate found
    if (tempReturn > 0)
    {
...
int KvhPacketStorage::AddPacket(packet_id_e _packetId)
{

  if (Contains(_packetId))
  {
    return 1;
  }
  ...
  */
  ...
  case packet_id_odometer_configuration:
    packetMap_[packet_id_odometer_configuration] = std::make_pair(false, std::make_shared<odometer_configuration_packet_t>());
    break;
  case packet_id_body_velocity:
    packetMap_[packet_id_body_velocity] = std::make_pair(false, std::make_shared<body_velocity_packet_t>());
    break;
  case packet_id_velocity_standard_deviation:
    packetMap_[packet_id_velocity_standard_deviation] = std::make_pair(false, std::make_shared<velocity_standard_deviation_packet_t>());
    break;
  default:
    return -1;
  }
